...
# Data preprocessing
df = df.drop(columns=['id', 'movie_title', 'genre', 'mpaa_rating',
                      'runtime_minutes', 'critic_score', 'audience_score',
                      'release_year'])

ss = StandardScaler()
fit_ss = ss.fit_transform(df[['total_gross', 'inflation_adjusted_gross']])
ss_df = pd.DataFrame(fit_ss,
                     columns=['total_gross', 'inflation_adjusted_gross'],
                     index=df.index)
...
dump(le_fit, 'save.joblib')  # save the trained model

app = Flask(__name__)

linear_reg_model = load('save.joblib')

@app.route('/model_prediction', methods=['POST'])
def predict():
    data = request.json['features']
    train_data = np.array(data).reshape(1, -1)
    predict_model = linear_reg_model.predict(train_data)[0]
